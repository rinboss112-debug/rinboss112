...
def _require_access_code(
    ...
    try:
        ...
        st.error(
            f"Không kiểm tra được quyền truy cập: {error}",
    ...
    if not policy["scraper_enabled"]:
        st.warning(
            "Admin đang tạm khóa chức năng cào sản phẩm.",
            icon=":material/lock:",
        )
        st.stop()

    authenticated_id = str(st.session_state.get("access_user_id", ""))
    user = next(
        (
            item
            for item in policy["users"]
            if item["id"] == authenticated_id and item["enabled"]
        ),
        None,
    )
    if user is None:
        ...
        with st.container(horizontal_alignment="center"):
            ...
            if st.button(
                ...
                type="primary",
                icon=":material/login:",
                key="access_code_login",
            ):
                try:
                    identity = store.authenticate(access_code)
                except Exception as error:
                    st.error(f"Không kiểm tra được mã truy cập: {error}")
                else:
                    if identity is None:
                        st.error("Mã truy cập không đúng hoặc đã bị khóa.")
                    else:
                        st.session_state.access_user_id = identity["id"]
                        st.session_state.access_user_name = identity["name"]
                        st.rerun()
        st.stop()

    today = datetime.now(ZoneInfo("Asia/Bangkok")).date().isoformat()
    used_today = int(user["usage"].get(today, 0))
    return {
        "id": user["id"],
        "name": user["name"],
        "used_today": used_today,
        "daily_run_limit": int(policy["daily_run_limit"]),
        "max_keywords_per_run": int(policy["max_keywords_per_run"]),
        "max_pages_per_niche": int(policy["max_pages_per_niche"]),
        "max_products_per_niche": int(policy["max_products_per_niche"]),
    }
...
def _load_local_history(requested_by_id: str = "") -> pd.DataFrame:
    ...
    for manifest_path in RUNS_DIR.glob("*/manifest.json"):
        try:
            ...
            run_owner_id = str(
                payload.get("settings", {}).get("requested_by_id", "")
            )
            if requested_by_id and run_owner_id != requested_by_id:
                continue
            rows.append(
                {
...
def _render_history_view(requested_by_id: str = "") -> None:
    history = _load_local_history(requested_by_id)
    if history.empty:
...
admin_secrets = _secrets_section("admin")
drive_ready = all(
    str(drive_secrets.get(key, "")).strip()
    for key in ("client_id", "client_secret", "refresh_token")
)
enforce_access_control = bool(admin_secrets.get("enable_access_control", False))
access_store = AccessControlStore(
    OUTPUT_DIR / "access_control.json",
...
with st.sidebar:
    ...
    if access_identity:
        with st.container(border=True):
            st.caption(f"Đang dùng mã của **{access_identity['name']}**")
            remaining_runs = max(
                access_identity["daily_run_limit"]
                - access_identity["used_today"],
                0,
            )
            st.caption(f"Còn {remaining_runs} lượt hôm nay.")
            if st.button(
                "Đăng xuất",
                icon=":material/logout:",
                key="access_code_logout",
            ):
                st.session_state.pop("access_user_id", None)
                st.session_state.pop("access_user_name", None)
                st.rerun()
    ...
    if access_identity:
        st.session_state["max_pages"] = min(
            int(st.session_state["max_pages"]),
            access_identity["max_pages_per_niche"],
        )
        st.session_state["max_products"] = min(
            int(st.session_state["max_products"]),
            access_identity["max_products_per_niche"],
        )
    ...
    max_pages = limit_columns[0].number_input(
        ...
        min_value=1,
        max_value=(
            access_identity["max_pages_per_niche"] if access_identity else None
        ),
        step=1,
        key="max_pages",
    )
    ...
    st.caption(
        "Nhận cả hai loại: Prime member phải có FREE + Today/Tomorrow/Overnight; "
        "Fresh được ghi chi tiết Ships from/Sold by trong cùng cột thông tin ship."
    )
    ...
    if not zip_code.strip():
        ...
        validation_error = (
            ...
            f"{access_identity['max_keywords_per_run']} ngách mỗi lượt."
        )
        ...
        if validation_error:
            ...
            if access_identity:
                try:
                    ...
                    allowed = False
                    access_message = f"Không ghi nhận được quota: {error}"
                if not allowed:
                    st.error(access_message, icon=":material/block:")
                    st.stop()
                st.toast(access_message, icon=":material/security:")
            run_id = _new_run_id()
            config = RunConfig(
                run_id=run_id,
                output_dir=RUNS_DIR / run_id,
...
def render_live_dashboard() -> None:
        ...
        with st.container(horizontal=True):
            ...
            if (
                ...
                if st.button(
                    ...
                    if access_identity:
                        try:
                            retry_allowed, retry_message = access_store.reserve_run(
                                access_identity["id"],
                                len(snapshot["failed_keywords"]),
                                live_controller.config.max_pages,
                                live_controller.config.max_products,
                            )
                        except Exception as error:
                            retry_allowed = False
                            retry_message = f"Không ghi nhận được quota: {error}"
                    if retry_allowed:
                        retry = _retry_config(
                        ...
                        st.error(retry_message, icon=":material/block:")
    ...
    if view == "Kết quả":
        ...
        _render_history_view((access_identity or {}).get("id", ""))
