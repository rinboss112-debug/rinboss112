...
class ImageGalleryExtensionTests(unittest.TestCase):
    ...
    def test_extension_supports_csv_gallery_workflow(self) -> None:
        ...
        self.assertIn('id="delay-seconds" type="number" min="1"', popup_html)
        self.assertIn("image_gallery_status", popup)
        ...
        self.assertIn("MIN_DELAY_SECONDS = 1", popup)
        self.assertIn("MIN_DELAY_SECONDS = 1", background)
        self.assertIn("normalizeDelaySeconds", popup)
        self.assertIn("normalizeDelaySeconds", background)
    ...
    def test_app_registers_and_renders_download_page(self) -> None:
        ...
        self.assertEqual("Tải Product Image Collector 1.1.5", app.download_button[0].label)
