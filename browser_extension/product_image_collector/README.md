# RinBoss Product Image Collector 1.1.3
...
2. Mở extension và chọn một trong hai cách nhập:
   - dán trực tiếp 1–100 link sản phẩm, mỗi dòng một link, rồi bấm **Nạp danh sách link**; hoặc
   - chọn CSV có một trong các cột `product_url`, `amazon_url`, `temu_url`, `url`, `link`; CSV Amazon cũng có thể chỉ cần cột `asin`.
3. Chọn số ảnh (mặc định 5), thời gian nghỉ rồi bấm **Bắt đầu lấy ảnh**. Với danh sách lớn, nên nghỉ 10–15 giây giữa sản phẩm.
...
Với Amazon, extension chỉ đọc ảnh trong khối thư viện sản phẩm (`/images/I/`). Logo Prime, banner, video và ảnh sản phẩm gợi ý ở phần khác của trang sẽ bị loại. Trước khi xuất, extension còn xóa ảnh cũ không hợp lệ và loại link trùng sau khi chuẩn hóa URL ảnh gốc.
